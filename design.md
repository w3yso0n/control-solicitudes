...
**En impresión** (`hidden print:block`, clases `print-report__*` y `print-cover__*` en `globals.css`): documento propio, no una copia de la vista de pantalla con `window.print()`. Página carta (`@page { size: letter portrait; margin: 16mm 16mm 14mm }`). Una sola familia tipográfica en todo el documento — Inter (`--font-sans`) — incluyendo los números grandes; nunca `font-display` (Fraunces) ni ninguna serif en el PDF: la jerarquía se hace con tamaño/peso, no con familia.

- **Portada** (`print-cover`, primera hoja, `break-after: page`): masthead editorial, no bloques de fondo de color. Regla: el color (guinda) vive en texto, bordes y reglas horizontales/verticales — **nunca solo en `background`**, porque el diálogo de impresión nativo trae "Gráficos en segundo plano" desactivado por defecto y cualquier fondo de color se vuelve invisible (texto blanco ilegible sobre blanco). Estructura: logo + meta de generación arriba, regla horizontal gruesa (`print-cover__rule`, 3pt guinda) como separador de marca, luego el masthead (`grid-template-columns: 1fr auto`, sin anchos fijos en `pt` que puedan desbordar y cortar el título — el título usa `overflow-wrap: break-word` y todo el ancho disponible de la columna): título del reporte grande (`font-weight: 800`, ~33pt) a la izquierda, KPI principal (volumen + delta) como número guinda gigante a la derecha con borde izquierdo delgado como separador. Debajo, `print-cover__stats` en 6 columnas separadas por hairlines verticales con una regla superior tinta. Cierra con `print-cover__highlights` (zona con más volumen / tema principal), cada uno con su propia regla guinda bajo el título de sección, y la leyenda legal al final. Todo debe caber en una sola hoja — nunca forzar `min-height`/`vh` cercano al alto de página en `@media print` (poco fiable entre navegadores y genera una hoja en blanco extra antes del cuerpo).
- **Cuerpo** (`print-report`, desde la segunda hoja): header simple (título + rango, sin repetir logo) y secciones con hairlines (`border-bottom` 0.5–1.5pt en grises zinc), sin `Card`, sin sombras, sin `rounded-2xl`. Tablas con cabecera uppercase pequeña y alineación numérica a la derecha.

`AppShell` oculta sidebar y header con `print:hidden` y quita el padding de `main` con `print:p-0` para que el documento ocupe toda la hoja.

Si se agrega una sección nueva al reporte, se agrega en los tres lugares que aplique: la versión escaneable en pantalla, el highlight de portada si es una métrica top-line, y su equivalente editorial en el bloque de impresión.
